...
export default function getVideoContainerClient(): ContainerClient {
    const account = import.meta.env.VITE_STORAGE_ACCOUNT  // get the storage account name from the .env file
    const containerName = import.meta.env.VITE_VIDEO_STORAGE_CONTAINER
    const sas = import.meta.env.VIDEO_STORAGE_SAS
